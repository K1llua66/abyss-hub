...
local function createToggle(parent, text, defaultValue, callback)
    ...
    toggleBtn.Size = UDim2.new(0, 40, 0, 28)
    toggleBtn.Position = UDim2.new(1, -48, 0, 6)
    toggleBtn.BackgroundColor3 = defaultValue and Color3.fromRGB(100, 200, 100) or Color3.fromRGB(80, 80, 100)
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        value = not value
        toggleBtn.BackgroundColor3 = value and Color3.fromRGB(100, 200, 100) or Color3.fromRGB(80, 80, 100)
...
local function createSlider(parent, text, min, max, defaultValue, callback)
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(1, -16, 0, 65)
    frame.BackgroundColor3 = Color3.fromRGB(45, 45, 55)
    frame.BackgroundTransparency = 0.5
    frame.BorderSizePixel = 0
    frame.Parent = parent
    ...
    label.BackgroundTransparency = 1
    label.Text = text .. " (" .. defaultValue .. "x)"
    label.TextColor3 = Color3.fromRGB(220, 220, 220)
    label.TextSize = 13
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.Font = Enum.Font.Gotham
    ...
    slider.Size = UDim2.new(1, -16, 0, 4)
    slider.Position = UDim2.new(0, 8, 0, 35)
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LP then
                ...
                if c and c:FindFirstChild("HumanoidRootPart") and c.Humanoid and c.Humanoid.Health > 0 then
                    addLabel(c, p.Name, p.TeamColor and p.TeamColor.Color or Color3.new(1, 0.5, 0.5))
                end
            end
        end
    end
        ...
        if enemies then
            for _, n in ipairs(enemies:GetChildren()) do
                if n:IsA("Model") and n:FindFirstChild("Humanoid") and n.Humanoid.Health > 0 then
                    addLabel(n, n.Name, Color3.new(1, 0.7, 0))
                end
            end
        end
    end
...
local function teleport(coords)
    if root then root.CFrame = CFrame.new(coords) end
end
...
-- PvP секция
createSection(scrollFrame, "PvP Functions")
...
createToggle(scrollFrame, "Speed Boost", false, function(v) state.speed_enabled = v; updateSpeed() end)
createSlider(scrollFrame, "Speed", 1, 10, 1, function(v) state.speed = v; if state.speed_enabled then updateSpeed() end end)
createToggle(scrollFrame, "Jump Boost", false, function(v) state.jump_enabled = v; updateJump() end)
createSlider(scrollFrame, "Jump", 1, 10, 1, function(v) state.jump = v; if state.jump_enabled then updateJump() end end)

-- ESP секция
createSection(scrollFrame, "ESP Functions")
createToggle(scrollFrame, "Player ESP", false, function(v)
    state.player_esp = v
    if state.player_esp or state.npc_esp or state.fruit_esp or state.chest_esp then startESP() else stopESP() end
end)
...
-- Настройки секция
createSection(scrollFrame, "Settings")
...
-- Кнопка для смены клавиши (простая)
local keyBtn = createButton(scrollFrame, "Change Keybind (Current: " .. state.window_bind .. ")", nil)
local keyOptions = {"RightShift", "LeftShift", "K", "L", "U", "I", "O", "P", "Q", "E", "R", "T", "Y"}
local keyIndex = 1
